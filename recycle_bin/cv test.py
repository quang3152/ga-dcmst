...
def create_new_population(population, fitness, pop_size):
    # Tạo danh sách cá thể cùng fitness
    individuals_fitness = [(population[i], fitness[i]) for i in range(len(population))]

    # Tạo quần thể mới với 1 nửa là cá thể tốt nhất
    ...
    # Danh sách cá thể còn lại (ngoài cá thể đã được chọn)
    remaining_individuals = individuals_fitness[pop_size // 2:]
    # Thêm 1 nửa quần thể còn lại bằng cách cho 2 cá thể ngẫu nhiên tranh đấu
    for _ in range(pop_size // 2):
        #         print("remaining", remaining_individuals)
        # chọn ngẫu nhiên 2 cháu
